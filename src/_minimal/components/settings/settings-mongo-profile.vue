...
const currentProfile = computed(() =>
  normalizeProfile(api.settings.get('mongoProfile') || 'profile1'),
);
...
async function applyProfile() {
  ...
  const skipConfirm = Boolean(api.settings.get('mongoProfileConfirmSkip'));
  const lastConfirmed = normalizeProfile(api.settings.get('mongoProfileLastConfirmed') || '');

  if (!skipConfirm && next !== previous && next !== lastConfirmed) {
    const checkboxId = 'mongo-profile-confirm-skip';
    ...
    await api.settings.set('mongoProfileLastConfirmed', next);
    if (skipWarning) {
      await api.settings.set('mongoProfileConfirmSkip', true);
    }
  }

  await api.settings.set('mongoProfile', next);
